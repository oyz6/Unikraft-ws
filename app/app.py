...
async def http_handler(request):
    ...
    if request.path == f'/{SUB_PATH}':
        ...
        vless = (f"vless://{UUID}@{domain}:{port}"
                 ...
                 f"&fp=chrome&type=ws&host={domain}&path=%2F{WSPATH}#{name_part}")
...
def _start_nezha_sync() -> bool:
    try:
        ...
        if not os.path.exists(local):
            if not _download_agent_so(): return False
...
def _write_template_instance():
    """
    尝试写入 template_instance。
    - 首次（模板准备）：阻塞直到平台完成快照，返回后即为新克隆实例
    - 之后（从快照恢复）：立即返回
    """
    try:
        ...
        with open(TEMPLATE_INSTANCE_PATH, 'w') as f:
            f.write('1')
        logger.info("template_instance write returned")
    except FileNotFoundError:
        logger.info(f"{TEMPLATE_INSTANCE_PATH} not found, skipping (not on Unikraft?)")
    except Exception as e:
        logger.warning(f"template_instance write failed: {e}")


# ==================== 主函数 ====================
async def main():
    # 1. 预热 aiohttp（快照会保存预热后的状态）
    _warmup_aiohttp()

    # 2. 写入 template_instance
    #    模板准备阶段阻塞，快照完成后返回；从快照恢复时立即返回
    _write_template_instance()
    ...
    if ENABLE_NEZHA:
        if NZ_SERVER and NZ_CLIENT_SECRET:
            ...
            logger.info("ENABLE_NEZHA=true but missing NZ_SERVER/NZ_CLIENT_SECRET")
    else:
