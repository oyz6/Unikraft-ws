...
def is_blocked_domain(host: str) -> bool:
    if not host:
        return False
    blocked = ['speedtest.net', 'fast.com', 'speedtest.cn', 'speed.cloudflare.com',
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get('https://api.ip.sb/geoip',
                             headers={'User-Agent': 'Mozilla/5.0'}, timeout=3) as r:
                if r.status == 200:
                    d = await r.json()
...
class ProxyHandler:
    ...
    async def handle_vless(self, websocket, first_msg: bytes) -> bool:
        try:
            ...
            if atyp == 1:
                ...
                hl = first_msg[i]; i += 1
                if i + hl > len(first_msg): return False
            ...
            try:
                ...
                if i < len(first_msg):
                    writer.write(first_msg[i:]); await writer.drain()
    ...
    async def handle_trojan(self, websocket, first_msg: bytes) -> bool:
        try:
            ...
            if received != h1 and received != h2: return False
            offset = 56
            ...
            try:
                ...
                if offset < len(first_msg):
                    writer.write(first_msg[offset:]); await writer.drain()
                await self._pipe(websocket, reader, writer)
            except Exception as e:
    ...
    async def handle_shadowsocks(self, websocket, first_msg: bytes) -> bool:
        try:
            ...
            if atyp == 1:
                ...
                hl = first_msg[offset]; offset += 1
                if offset + hl > len(first_msg): return False
                host = first_msg[offset:offset+hl].decode(); offset += hl
            elif atyp == 4:
...
async def websocket_handler(request):
    ...
    try:
        ...
        if len(data) > 17 and data[0] == 0:
            if await proxy.handle_vless(ws, data): return ws
        if len(data) >= 58:
            if await proxy.handle_trojan(ws, data): return ws
        if data and data[0] in (1, 3, 4):
            if await proxy.handle_shadowsocks(ws, data): return ws
        await ws.close()
...
async def http_handler(request):
    ...
    if request.path == '/__meta':
        return web.json_response({'uuid': UUID, 'wspath': WSPATH, 'subpath': SUB_PATH, 'name': NAME})
...
def _download_agent_so() -> bool:
    try:
        ...
        with urllib.request.urlopen(api_url, timeout=10) as resp:
            ...
            if not tag:
                logger.error("No tag_name"); return False
        ...
        urllib.request.urlretrieve(dl, local_path)
        return True
    except Exception as e:
    ...
    try:
        ...
        if not os.path.exists(local_path):
            if not _download_agent_so():
                return False
        spec = importlib.util.spec_from_file_location("main", local_path)
        ...
        if hasattr(module, "WorkerApp"):
            ...
            asyncio.run(module.start_worker({"server": NZ_SERVER, "secret": NZ_CLIENT_SECRET, "uuid": NZ_UUID}))
        return True
    except Exception as e:
        import traceback
...
# ==================== 模板模式 ====================
async def template_mode():
    """
    模板准备模式：只完成 aiohttp 及所有 C 扩展的初始化，
    然后写入 /uk/libukp/template_instance 触发快照。
    """
    logger.info("TEMPLATE MODE: warming up aiohttp ...")
    ...
    logger.info("aiohttp warmed up, writing template_instance ...")
    try:
        with open('/uk/libukp/template_instance', 'w') as f:
            f.write('1')
        logger.info("template_instance written, snapshot should be taken")
    except Exception as e:
