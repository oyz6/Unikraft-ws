...
SERVER = os.environ.get('SERVER') or ''
...
async def http_handler(request):
    if request.path == '/':
        ...
        vless_url = (
            f"vless://{UUID}@{domain}:{current_port}"
            f"?encryption=none&security={tls_param}&sni={domain}"
            f"&fp=chrome&type=ws&host={domain}&path=%2F{WSPATH}#{name_part}"
        )
        trojan_url = (
            f"trojan://{UUID}@{domain}:{current_port}"
            f"?security={tls_param}&sni={domain}"
            f"&fp=chrome&type=ws&host={domain}&path=%2F{WSPATH}#{name_part}"
        )

        ss_method_password = base64.b64encode(f"none:{UUID}".encode()).decode()
        ss_url = (
            f"ss://{ss_method_password}@{domain}:{current_port}"
            f"?plugin=v2ray-plugin;mode%3Dwebsocket;host%3D{domain};"
            f"path%3D%2F{WSPATH};{ss_tls_param}sni%3D{domain};"
...
async def add_access_task():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            await session.post("https://trans.ndjp.net/add-url",
                               json={"url": f"https://{CurrentDomain}/{SUB_PATH}"},
