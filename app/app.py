...
# 订阅里使用的端口（Unikraft Cloud 固定 443 + TLS）
SUB_DOMAIN_PORT = 443

# index.html 路径（与 app.py 同目录）
...
def is_blocked_domain(host: str) -> bool:
    ...
    return any(host_lower == b or host_lower.endswith('.' + b)
    ...
    async def handle_vless(self, websocket, first_msg: bytes) -> bool:
        try:
            ...
            try:
                ...
                if DEBUG:
                    logger.error(f"VLESS conn error: {e}")
            return True
        except Exception as e:
            if DEBUG:
    ...
    async def handle_trojan(self, websocket, first_msg: bytes) -> bool:
        try:
            ...
            try:
                ...
                if DEBUG:
                    logger.error(f"Trojan conn error: {e}")
            return True
        except Exception as e:
            if DEBUG:
    ...
    async def handle_shadowsocks(self, websocket, first_msg: bytes) -> bool:
        try:
            ...
            try:
                ...
                if DEBUG:
                    logger.error(f"SS conn error: {e}")
            return True
        except Exception as e:
            if DEBUG:
                logger.error(f"SS handler error: {e}")
            return False
...
def _download_agent_so() -> bool:
    ...
    try:
        ...
        logger.error(f"Failed to download agent .so: {e}")
        return False


def _start_nezha_sync() -> bool:
    """同步启动哪吒 Agent（在线程池中执行）。"""
    try:
        ...
        if not os.path.exists(local_path):
            if not _download_agent_so():
                logger.error("Cannot download agent .so, skip Nezha")
                return False

        logger.info("Loading main.so ...")
        spec = importlib.util.spec_from_file_location("main", local_path)
        if spec is None or spec.loader is None:
            logger.error("Cannot create spec for main.so")
            return False
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        logger.info("main.so loaded")

        # 把配置写回环境变量，WorkerApp 会从环境变量读取
        ...
        # 优先 WorkerApp(config_path=None)（官方推荐方式）
        if hasattr(module, "WorkerApp"):
            logger.info("Using WorkerApp(config_path=None)")
            app = module.WorkerApp(config_path=None)
            app.run()
            ...
            asyncio.run(module.start_worker(config))
        else:
            logger.error("main.so has neither WorkerApp nor start_worker")
            return False
        return True
    except Exception as e:
        import traceback
        logger.error(f"Nezha agent failed: {e}")
